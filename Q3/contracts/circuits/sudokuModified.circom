...
template sudoku() {
    ...
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            rproof[count] = RangeProof(32);
            rproof[count].range[0] <== 0;
            rproof[count].range[1] <== 9;
            rproof[count].in <== puzzle[i][j];
            assert(rproof[count].out == 0);

            sproof[count] = RangeProof(32);
            sproof[count].range[0] <== 0;
            sproof[count].range[1] <== 9;
            sproof[count].in <== solution[i][j];
            assert(sproof[count].out == 0);
